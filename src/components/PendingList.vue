      ...
      <table class="min-w-full leading-normal">
        ...
        <tbody>
          <tr v-for="item in getContacts" :key="item.id" >
            ...
            <td class="px-5 py-1 border-b border-gray-200 bg-white text-sm" v-if="!item.completed">
              <div class="flex">
            ...
            <td class="px-2 py-2 border-b border-gray-200 bg-white text-sm" v-if="!item.completed">
              <p v-bind:class = "(new Date(item.dueDate)<new Date())?'text-red-300':'normal'" class="text-gray-900 whitespace-no-wrap">{{ item.dueDate }}</p>
            </td>
            <td class="px-2 py-2 border-b border-gray-200 bg-white text-sm" v-if="!item.completed">
              <p class="text-gray-900 whitespace-no-wrap">{{ item.completed?"completed":"not completed" }}</p>
            </td>
